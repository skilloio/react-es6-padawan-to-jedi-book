...
arquivo `react.js` e deve ser incluído depois de já ter incluído arquivo `reactU.js`.

file is the core file needed to create React elements and write react components. When you intend to render your components in an HTML document (i.e., the DOM) you'll also need the `react-dom.js` file. The `react-dom.js` file is dependent on the `react.js` file and must be included after first including the `react.js` fi
